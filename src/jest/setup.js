...
const suppressedErrors = /(React does not recognize the.*prop on a DOM element|Unknown event handler property|is using incorrect casing|is using uppercase HTML|Received `true` for a non-boolean attribute `accessible`|Invalid value for prop %s on <%s> tag|Received `%s` for a non-boolean attribute `%s`|The tag.*is unrecognized in this browser)/;
